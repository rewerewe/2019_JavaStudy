...
/*
 ○ 중첩 클래스 개요 
	...
	클래스를 중첩하여 프로그램 구조를 보다 더 간단하고 알아보기 쉽도록 만들 수 있는데
	이러한 클래스를 중첩 클래스라 한다.
 ...
 ○ 중첩 클래스의 종류(4가지)
	...
	3.	지역 클래스(로컬 클래스, local class) - 메소드 안에 객체가 구성
	...
	4. 익명 클래스(무명 클래스, anonymous class) - 종이컵 
	   이름 없는 클래스
	   
	   - 클래스 또는 인터페이스에 대한 객체를 생성하면서
	     바로 클래스 또는 인터페이스를 정의한ㄴ 클래스
	   - 정의하는 부분과 생성하는 부분이 하나로 묵ㅇ여져 
...
class Test
{
	...
	public static class StaticNested				// static 1) 탄생시점 2) 공유 
	{
		...
		void write()
		{
			...
			// System.out.println("b : " + b);			//-- 에러 발생 : non-static 
			System.out.println("c : " + c);			// 변수 c와 쌍둥이 
