...
// 평시와 비상시의 우선순위가 다르듯이, 
// 보편적으로 많이 사용하는 클래스를 먼저 정리하는 것이다.
...
/*
 ...
 ○ import 문
	...
	import만 되어 있기 때문에 이 패키지의 클래스들은 import 하지 않고 
	바로 사용하는 것이 가능하다. 
	...
	import [static] 패키지.*;
	 ...
	 ※ JDK 1.5 부터는 import 다음에 『static』키워드를 붙임으로써
		...
		만약 패키지를 정적으로 import 한 경우라면 모두 접근 가능한 
		메소드와 멤버 변수들은 접두사(ex. ob, sc, br)를 붙이지 않고 
		사용하는 것이 가능하다.

 ○ Object 클래스
	『java.lang.Object』 클래스는 자바 클래스의 최상의 클래스로 
...
public class Test121 // extends Object
{
	...
	*/
	...
	public static void main(String[] args)
	{
		...
		// 임의로생성된객체.xxx();		//-- 대상 클래스가 갖고 있는 메소드xx() 호출
		//-- 해당 객체를 생성시키는 대상 클래스의 메소드

		// ob.test();
		//--==>> 에러 발생 : 해당 클래스에 test() 라는 메소드가 정의되어 있지 않기 때문에.

		// System.out.println(ob.action());
		//--==>> 에러 발생 : 해당 클래스에 action() 라는 메소드가 정의되어 있지 않기 때문에.

		System.out.println(ob.toString());
		//--==>> Test121@15db9742
		//-- Test해시코드 : 자바가 객체를 구분하기 위해 임의로 부여하는 식별 번호 
